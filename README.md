...
- **AI 数据提取** — LLM 自动提取血清阳性率/GMC 等数据点，支持 DeepSeek/OpenAI/Qwen/本地 Ollama，长文档分块并行提取，精确字符级溯源，强 Schema 校验；管线加固（API Key 加密存储、提示注入防护、token 预算/日配额/并发上限、幂等写库、状态超时自动回收）；「提取状态」弹窗与列表查询统计口径统一，区分「已完成/完成（无数据）/失败」三终局
