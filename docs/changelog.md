...
- **省份分区 / 简称元数据**：新增中国 34 个省级行政区（省/自治区/直辖市/特别行政区）的分区（中部/东部/西部/北部/南部）与简称（京、沪……）权威元数据模块 `backend/app/core/reference_data/china_provinces.py`，提供 `province_region` / `province_abbr` / `province_full_name` / `group_provinces_by_region` 等查询函数，供字典 API、区域聚合与前端选择器消费；元素「只增不改」，向后兼容既有标准化与聚合逻辑。
- **省份字典扩展**：`/dictionary/provinces` 每个省级元素追加 `region`、`abbr` 字段（保留 `code`/`name` 不变）。
- **区域对比补充分区**：区域对比分析每省结果新增 `region` 字段；省份筛选器与分析页区域对比表格展示「简称·分区」（如「北京（京·中部）」）。
- **分区对比分析**：数据分析新增「分区汇总」表格与接口 `GET /analysis/zone-compare`，将已审核数据点按所属分区（中部/东部/西部/北部/南部）归组做区内 Meta 合并，输出每区综合阳性率 / GMC / 样本量及成员省份，作为省级区域对比的补充汇总。
- **AI 准确度自测（合成文献验证）**：文件夹监控页新增「AI 准确度自测」——配置本地生成模型 A，程序先按已知答案生成 ground truth（含血清阳性率 + GMC 数据点，随机种子可复现），由模型 A 批量生成学术风格合成文献（按比例混入越界值/缺字段/格式变异/完全错误值四类噪声）并直接入库；再选择提取模型 B 走真实 AI 提取链路提取，与 ground truth 比对输出精确匹配率、容差匹配率、噪声拒绝率及字段级/分噪声类型明细，支持 CSV 导出。合成文献带 `is_synthetic` 标记，统计时默认排除，不污染正式数据。
- **KG 定向抽取登记优化**：定向抽取任务提交后在后端立即登记任务状态（queued），前端对状态轮询的瞬时 404 增加连续 4 次重试容忍，避免 worker 并发=1 排队时把「尚未登记」误判为「任务过期」而报错终止。
- **任务状态接入自测**：系统设置「任务状态」新增「AI准确度自测」分组，显示自测生成的运行/空闲状态与进行中数量，后台任务一屏全面掌握。

### 修复

- **LLM 请求超时默认值**：`LLM_REQUEST_TIMEOUT` 默认由 600s 提高至 1200s（20 分钟），适配本地大模型（如 qwen2.5:32b）长文提取的慢推理，减少读超时导致的提取失败。
- **系统设置标签布局**：调整各功能标签间距并允许换行，6 个功能（任务状态/远程模型/本地模型/数据备份/后台日志/系统信息）一屏全部可见，「系统信息」不再被折叠隐藏。

## v1.23.0 (2026-09-04)

### 新增

- **知识图谱智能问答**：知识图谱页新增「咨询问答」，支持开放领域自然语言提问，回答自动绑定数据点溯源。内置模板优先命中（地区+疾病+阳性率、作者/单位查询、疫苗咨询等），未命中时走 LLM 兜底并在回答前注入真实数据点作为证据，禁止编造数值。
- **问答地点识别强化**：支持省/市/区县与国外拉丁地名（`_LOC` 正则引入拉丁字母分支），地区匹配采用精确 + 部分匹配策略（兼容「北京」与「北京市」），剥离省/市/区尾缀。
- **作者/单位查询**：新增 `author_query` 模板（优先级最高），支持「北京麻疹研究的作者都有谁」「丰台麻疹研究论文的作者是谁」等三类句式，通过关联数据点定位地区并从文献表提取作者，文献数 ≤15 时附加清单。
- **KG 定向抽取（从文献列表选择）**：知识图谱「定向抽取」支持直接从文献列表勾选目标（`LiteraturePicker`：按省份筛选、关键词搜索、跨页累计勾选），与粘贴 ID 去重合并后作为抽取目标；文献列表新增「KG 抽取」列与筛选（显示已抽取数 / 未抽取）。
- **本地模型「已下载」检测列**：本地模型配置表格新增「已下载」列，调用 Ollama `/api/tags` 实时检测模型是否已下载（已下载绿 / 未下载红 / 探测失败未知橙），从源头预警「已启用但本地未安装」的错配。
- **模型未安装预检**：本地 Ollama 提取前通过 `/api/tags`（30s TTL 缓存）校验模型是否已下载，未安装直接以确定性错误 `[model_not_installed]` 返回并提示 `ollama pull`，不再触发无意义重试。
- **报告后台异步生成**：报告生成改为「点外卖」模式——提交后台异步任务后前端原地轮询进度并自动展示，长报告不再因请求超时中断；报告下载（md/docx/pdf）带 JWT 鉴权。
- **配置变量化**：`docker-compose.yml` 凭据与路径统一改为环境变量引用——`DATABASE_URL` 显式覆盖为容器内 `postgres:5432` 服务名、监控目录改为 `MONITORED_REF_HOST_PATH`、`GRAFANA_ADMIN_PASSWORD` 强制要求，避免硬编码。

### 修复

- **LLM 调用错误归类**：兜底调用不再用「连接失败」覆盖首个确定性错误（404/5xx/鉴权/JSON），`model not found` 等按 `http_4xx` 直接判 failed，杜绝 `15s×4` 的无意义重试双倍计费；移除永远连不上的死地址 `host.docker.internal`。
- **KG 抽取外键错误**：此前永久删除文献后 `.txt` 缓存残留，后台 KG 抽取会以已删除的 `literature_id` 插入实体触发 `ForeignKeyViolationError`。现已加防御性跳过（记录不存在则跳过），并在永久删除/清空回收站时清理 `.txt` 缓存与关联 `kg_entity` 记录，从源头杜绝。
- **数据库连接健壮性**：`idle_in_transaction_session_timeout` 由 120s 延长至 600s（`base.py`），结合既有 `pool_pre_ping` 与 LLM 推理前 `commit` 归还连接，规避长 LLM 推理期间连接被服务端断开的写库失败。
- **视觉提取（VL）分批**：新增 `VL_BATCH_SIZE=6` 配置，把扫描页图片分批发送给本地视觉模型，避免一次性塞入过多图片超出 Ollama 上下文窗（32768 token）。
- **模型管理弹窗交互**：本地/远程「添加/编辑模型」子弹窗 `zIndex` 高于父弹窗（1100>1000），不再需要先关闭父窗口才能看到子窗；两个弹窗标题栏支持按住鼠标左键拖动移动。

### 优化

- 远程模型管理弹窗与本地模型一致支持层级与拖动。
- 更新 `process_literature` 在 `skipped_race/superseded` 状态的日志处理，避免 `KeyError`。
...
- **知识图谱**：13 种实体类型（病原体/地区/人群/检测方法/疫苗/调查/时间/指标/实施单位/作者/样本/数据质量/出版物）和 18 种关系类型的多维语义网络本体定义。
...
- **数据库表**：新增 `kg_entity` 和 `kg_triple` 表（Alembic 迁移 `add_kg_tables`）。
- **API 端点**：新增知识图谱专用 API——概览、筛选选项、图谱数据、实体搜索、路径推理、统计、批量抽取。
- **前端页面**：新增知识图谱可视化页面，包含关系图谱（ECharts 力导向图）、实体搜索、路径推理三个标签页；导航栏新增「知识图谱」入口，支持中英文 i18n。

### 修复

- **F14 连接池修复**：心跳超时保护（`_stop_heartbeat()` 改为 10s 有限等待）、写库前停止心跳消除行锁竞争、恢复 `pool_pre_ping` 避免长调用后复用陈旧连接、数据库连接断开错误归入 `connection_error` 快速重试。
- **pdf_parser.py**：修复 `asyncio.run(_run)` 缺少括号的调用错误。
...
- **扫描版文献 OCR 识别**：文字层缺失或损坏的扫描 PDF 自动触发 Tesseract OCR（中文 chi_sim + 英文 eng 双语言）识别；支持自动探测 Tesseract 安装位置与语言包目录，并新增 `TESSERACT_CMD` / `TESSERACT_DATA_DIR` 配置项，安装与配置详见 [tesseract_setup](tesseract_setup.md)。
