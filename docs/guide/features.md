...
2. **LLM 抽取**：从文献全文自动抽取三元组（头实体-关系-尾实体）持久化存储。此功能由 .env 中 ENABLE_KG_EXTRACTION=true 特性开关控制，**默认关闭**（避免用户误触产生额外 LLM 费用），关闭时触发 KG 的 API 直接返回 400 拒绝；失败不影响主提取流程
...
1. 进入「文件夹监控」→「AI 准确度自测」
2. 新建自测任务：选择**疾病**、**篇数**、**每篇数据点数**、**生成模型 A**、**噪声比例**（如 0.2）、随机**种子**，以及文献来源与载体选项
3. 点击「开始生成」，后台用模型 A（或参考模型）产出 GT 与合成文献并直接入库
4. 选择**提取模型 B**（可多选）→ 点击「开始提取」，复用现有 AI 提取链路
5. 待全部提取完成后点击「执行评估」，查看结果：
   - **精确匹配率**：字段完全一致的占比
   - **容差匹配率**：数值允许 ±5%（或比例误差）视为匹配
   - **噪声拒绝率**：噪声点应被拒绝/纠正而未被误采的比例
   - 字段级准确度矩阵、分噪声类型表现、逐文献明细；多模型对比时另输出模型间水平对比
6. 任务历史可回看，报告支持 **CSV 导出**
...
- **版本号自动解析**：后端启动时读取 docs/changelog.md 的首个 ## vX.Y.Z 标题作为 APP_VERSION，与 GitHub changelog 保持同步；修改 changelog 后重启容器即可更新系统设置里的版本号显示
